...
class AKConv(nn.Module):
    ...
    def forward(self, x):
        ...
        device = offset.device  # 以offset的设备为基准
        dtype = offset.dtype
        N = offset.size(1) // 2
        # (b, 2N, h, w)
        ...
        x_q_lt = self._get_x_q(x, q_lt.to(device), N)
        x_q_rb = self._get_x_q(x, q_rb.to(device), N)
        x_q_lb = self._get_x_q(x, q_lb.to(device), N)
        x_q_rt = self._get_x_q(x, q_rt.to(device), N)
